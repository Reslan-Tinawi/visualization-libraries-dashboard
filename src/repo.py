...
class Repo:
    ...
    def get_contrib_features(self) -> pd.DataFrame:
        ...
        contributor_features = [
            [self.repo.id, contrib.id, contrib.name, contrib.company, contrib.location, contrib.get_repos().totalCount]
            for contrib in contributors]

        df = pd.DataFrame(data=contributor_features, columns=['repo_id', 'contrib_id', 'contrib_name',
    ...
    def get_repo_releases(self) -> pd.DataFrame:
        ...
        releases_features = [[self.repo.id, release.tag_name, release.created_at.strftime('%d-%m-%Y')] for release in
                             releases]

        df = pd.DataFrame(data=releases_features, columns=['repo_id', 'tag_name', 'created_at'])
